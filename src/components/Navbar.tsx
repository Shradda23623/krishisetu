...
import { ShoppingCart, User, Menu, Search, Globe, X, Sun, Moon } from "lucide-react";
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navLinks.map(l => (
            <Link
              key={l.to}
              to={l.to}
              className="relative rounded-lg px-4 py-2 text-sm font-medium text-muted-foreground transition-all hover:text-foreground group"
            >
              {l.label}
            ...
            <DropdownMenuContent align="end" className="glass min-w-[180px]">
              {languages.map(lang => (
                <DropdownMenuItem
                  ...
                  className={language === lang.code ? "bg-primary/10 font-semibold text-primary" : ""}
                >
